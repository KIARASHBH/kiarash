local sudomsg = 300360749 -- put your id here
...
if msg.media then
  if msg.media.type:match("contact") then
    ...
    if not pm then 
	 return reply_msg(msg.id,'اد شدی پیوی باش', ok_cb, false)
	 else
...
ست کردن پروفایل ربات[با ریپلای]
💠!amar💠
دریافت آمار تب سین
💠!addmember💠
...
💠!setpm [text]💠
تنظیم پیام ادشدن مخاطب
💠!reload💠
بارگذاری مجدد تب سین
💠!addsudo [id]💠
اضافه کردن مدیر
💠!remsudo [id]💠
حذف مدیر
...
return text
  ...
  if matches[1] == "tick" then
    if matches[2] == "on" and is_sudo(msg) then
      redis:set("bot:tick", "on")
      return "tick > on"
    end
